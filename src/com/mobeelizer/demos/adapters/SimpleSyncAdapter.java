...
/**
 * An adapter for a list view in "Simple Sync" example.
 * 
 * @see MyArrayAdapter
 * @see ArrayAdapter
 * @see SimpleSyncEntity
 */
public class SimpleSyncAdapter extends MyArrayAdapter<SimpleSyncEntity> {
     ...
     * @param objects
     */
    public SimpleSyncAdapter(final Context context, final List<SimpleSyncEntity> objects) {
        super(context, R.layout.itemized_list_item, R.id.listItemText, objects);
    ...
    public View getView(final int position, View convertView, final ViewGroup parent) {
        ...
        // get current entity
        SimpleSyncEntity sse = getItem(position);

        // set the title
        vh.text.setText(sse.getTitle());
        // and an owner
        UserType user = UserType.valueOf(sse.getOwner());
        switch (user) {
            case A:
        ...
        if (sse.getEntityState() != EntityState.NONE) {
            Animation anim = null;
            int delay = 0;
            switch (sse.getEntityState()) {
            ...
            sse.setEntityState(EntityState.NONE);
        }
